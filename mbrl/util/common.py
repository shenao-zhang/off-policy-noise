...
def step_env_and_add_to_buffer_perturbed(
    env: gym.Env,
    obs: np.ndarray,
    agent: mbrl.planning.Agent,
    agent_kwargs: Dict,
    ...
    replay_buffer.add(obs, action, next_obs, reward, done)
    """
    size_scale = obs.shape
    #size_scale = 1
    scale = np.random.uniform(0.7, 1.3, size_scale)
    #var = 0.05
    #obs_noise = np.random.normal(0, var, obs.shape)
    obs_perturb = obs * scale
    #next_obs_noise = np.random.normal(0, var, obs.shape)
    next_obs_perturb = next_obs * scale
    sac_buffer.add(obs_perturb, action, next_obs_perturb, reward, done)
    """
    ...
    var = 1e-2 # 1e-1
    var_r = 0.4  # 0.5
    obs_perturb = obs + np.random.normal(0, var, obs.shape)
    act_perturb = agent.act(obs_perturb, **agent_kwargs)
    next_obs_perturb = next_obs + np.random.normal(0, var, obs.shape)
    reward_perturb = reward + np.random.normal(0, var_r)
    replay_buffer.add(obs_perturb, act_perturb, next_obs_perturb, reward_perturb, done)
    
    #var = np.abs(obs) / 30.0
    #var_r = np.abs(reward) / 30.0
    #obs_perturb = obs_perturb + np.random.normal(0, var, obs.shape)
    #act_perturb = agent.act(obs_perturb, **agent_kwargs)
    #next_obs_perturb = next_obs_perturb + np.random.normal(0, var, obs.shape)
    #reward_perturb = reward_perturb + np.random.normal(0, var_r)
    replay_buffer.add(obs_perturb, act_perturb, next_obs_perturb, reward_perturb, done)

    # calculate next perturbed state
   # env_dummy.reset()
   # env_dummy.state_vector()[:] = agent_obs
   # env_dummy.set_state(env_dummy.sim.get_state())
   # env_dummy.set_state(obs)
   # s_next_pert, reward_pert, done_pert, info_pert = env_dummy.step(action)
   # print(s_next_pert - next_obs)
    """
    if callback:
        callback((obs, action, next_obs, reward, done))
    return next_obs, reward, done, info
